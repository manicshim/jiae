...
print("| --- | ---: | ---: | ---: | ---: |")
print(
    f"| 2026-01-26 | {len(analyzed_tasks)}건 | "
    f"{len(risk_tasks)}건 | {len(analyzed_tasks) - len(risk_tasks)}건 | "
    f"{sum(task['workload_hours'] for task in analyzed_tasks)}시간 |"
)
print()
print("## 위험 작업")
print()
...
for task in risk_tasks:
    print(
        ...
        f"{task['remaining_days']}일 | {task['required_days']:.1f}일 | 위험 |"
    )
...
for index, task in enumerate(sorted_tasks, start=1):
    print(f"| {index} | {task['name']} ({task['task_id']}) | {task['priority']} | {task['due_date']} |")
...
| 기준일 | 전체 작업 수 | 위험 작업 수 | 정상 작업 수 | 총 작업 시간 |
| --- | ---: | ---: | ---: | ---: |
| 2026-01-26 | 5건 | 1건 | 4건 | 40시간 |
...
| 순위 | 작업 | 우선순위 | 마감일 |
| ---: | --- | --- | --- |
| 1 | 작업C (T003) | urgent | 2026-01-28 |
| 2 | 작업D (T004) | high | 2026-01-27 |
| 3 | 작업A (T001) | high | 2026-01-30 |
| 4 | 작업B (T002) | medium | 2026-02-05 |
| 5 | 작업E (T005) | low | 2026-02-10 |
```
